...
def initialize_marbling_with_colors():
    ...
    if bpm > 0 and len(circles) > 0:
        ...
        # --- 修正後の「崩し」ロジック ---
    if bpm > 0 and len(circles) > 0:
        # BPM（テンポ）を波の「細かさ（周波数）」に、
        # 音符数を波の「深さ（振幅）」に反映させる
        wave_frequency = bpm / 500.0  # 値が大きいほど波が細かくなる
        wave_amplitude = MARBLE_BREAK_INTENSITY * (len(midi_notes) / 10.0)

        print(f"BPMに基づき、横方向の波で全体を歪ませます... (Freq: {wave_frequency:.3f})")

        for c in circles:
            new_points = []
            # 各円の構成点を一つずつずらす
            for (px, py) in c.points:
                # y座標(py)を基準に、x方向(px)をサイン波でずらす
                # これにより「上から下へ流れる波」のような視覚効果が出る
                shift_x = sin(py * wave_frequency) * wave_amplitude
...
class manage_window:
    ...
    def mathematical(c: tuple, p: tuple, r: float):
        ...
        # 1. 基本の押し出し（これは層を作るために維持）
        distortion = sqrt(1 + (r**2 / (euc2 + 1e-6)))
        
        # 2. 【変更点】渦巻き(Swirl)を削除し、横方向の波(Wave)を生成
        # y座標(p[1])に応じてx方向のずれを作る
        wave_freq = 0.05  # 波の細かさ
        wave_amp = 10.0   # 波の強さ
        wave_shift = sin(p[1] * wave_freq) * wave_amp * (r / (euc + r))
        
        # 押し出しを適用した座標
        rx = dx * distortion + wave_shift # x方向にのみ波を加える
        ry = dy * distortion
