...
# モデルで使われたクラス順（固定、学習時の順序）
# この順序はモデルの出力インデックスと対応します。変更しないでください。
MODEL_GENRES = ['blues', 'classical', 'country', 'disco', 'hiphop', 'jazz', 'metal', 'pop', 'reggae', 'rock']

# 視覚用のジャンル順と色（ここを並べ替えると背景色の順序のみ変わる）
# デフォルトはモデル順に一致させてあるので、色だけ変えたい場合は
# 下の `VISUAL_GENRE_ORDER` の順序を変えてください（`MODEL_GENRES` は変更しない）。
...
if remainder != 0 and len(y) > window_size // 2:
    ...
    try:
        ...
        try:
            ...
            pred_genre = f"idx:{pred_idx}"
        print(f"Remainder {start_sec:.2f}s-{end_sec:.2f}s -> {pred_genre} (idx={pred_idx}, conf={pred_conf:.3f})")
    except Exception:
...
if results:
    ...
    try:
        ...
        final_genre = f"idx:{predicted_label}"
    print(f"検出されたジャンル: {final_genre}")
else:
        ...
        for i, x_break in enumerate(x_positions):
            ...
            for c in sorted_circles:
                ...
                if dist < break_range:
                    ...
                    # 波打ち成分も強度に応じて増やす（float のまま扱う）
                    wave = 5.0 * MARBLE_BREAK_INTENSITY * sin((c.position[0] / window_width) * 2 * pi)
                    total_shift = this_shift + wave
                    # 新しい実装:
                    # - 主変形はX方向（total_shift に依存）で与え、空間的モジュレーションと小さな旋回を追加
                    cx, cy = c.position
                    new_points = []
                    phase = i * 0.6
                    for (px, py) in c.points:
                        ...
                        base_amount = (total_shift / denom) * attenuation * global_factor

                        # 空間的モジュレーションで自然な波を付加
                        spatial_mod = 1.0 + 0.35 * sin((px / window_width) * 4.0 * pi + phase)
                        disp_x = base_amount * spatial_mod * (1.0 + 0.25 * MARBLE_BREAK_INTENSITY) * DISPLACEMENT_SCALE

                        # 縦方向の副次的変位（X変位に対して小さめ）
                        disp_y = 0.12 * base_amount * cos((py / window_height) * 3.0 * pi + phase) * (DISPLACEMENT_SCALE * 0.6)

                        # 旋回（swirl）：点を中心周りに少し回転させる（DISPLACEMENT_SCALEで微調整）
                        swirl_strength = 0.02 * attenuation * MARBLE_BREAK_INTENSITY * DISPLACEMENT_SCALE
                        ang = atan2(rel_y, rel_x)
                        r = sqrt(rel_x * rel_x + rel_y * rel_y)
                        new_ang = ang + swirl_strength * sin((px / window_width) * 2.0 * pi + phase)
                        rot_x = cos(new_ang) * r
                        rot_y = sin(new_ang) * r

                        new_px = cx + rot_x + disp_x
                        new_py = cy + rot_y + disp_y
...
class manage_window:
    ...
    def mathematical(c: tuple, p: tuple, r: float):
        ...
        # 1. 基本の押し出し (Mathematical Marbling)
        # 既存の点を外側に押し広げる
        distortion = sqrt(1 + (r**2 / (euc2 + 1e-6)))
        
        # 2. 混ざりながら伸びる回転 (Vortex Swirl)
        # 中心に近いほど強く回転させることで「伸び」を表現
        strength = 0.5  # 混ぜる強さ。大きくするとより渦巻く
        ...
        # 押し出しと回転を同時に適用
        rx = (dx * c_val - dy * s) * distortion
        ry = (dx * s + dy * c_val) * distortion
    ...
    def apply_fluid_drag(circle_obj, direction='horizontal'):
        ...
        for i in range(len(circle_obj.points)):
            ...
            if direction == 'horizontal':
                new_px = px + 15 * sin(py / 50.0 + phase)
                circle_obj.points[i] = (new_px, py)
            else:
                new_py = py + 15 * sin(px / 50.0 + phase)
                circle_obj.points[i] = (px, new_py)

    @staticmethod
    def calculate_new_circles(new_circle):
        # 新しいインクが落ちたとき、既存のすべての円の形を歪ませる
        circle_i = new_circle
        for circle_ii in circles:
            if not circle_ii == circle_i:
                for index in range(0, len(circle_ii.points)):
                    temp = manage_window.mathematical(
                        circle_i.position, 
                        circle_ii.points[index], 
                        circle_i.circle_radius
                    )
    ...
    def draw_circle_at_position(x: float, y: float, color_rgb: tuple):
        ...
        # 追加：新しいインク自体も少し歪ませて「伸び」を出す
        if random.random() > 0.5:
            manage_window.apply_fluid_drag(new_circle, random.choice(['horizontal', 'vertical']))
