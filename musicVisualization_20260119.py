            ...
            for c in sorted_circles:
                ...
                if dist < break_range:
                    ...
                    for (px, py) in c.points:
                        ...
                        # 横方向の伸長を強めるため、MARBLE_BREAK_INTENSITY に比例して倍率を増加
                        stretch_multiplier = 1.0 + 0.5 * MARBLE_BREAK_INTENSITY
                        stretch = 1.0 + stretch_amount * stretch_multiplier
                        # 縦方向の押しつぶしを小さくして控えめにする（完全に0にしない）
                        compress = 1.0 - 0.1 * global_factor * (1.0 + 0.3 * MARBLE_BREAK_INTENSITY)
                        # leading edge は横方向でより強調する（縦には影響を抑える）
                        thickness_scale = 1.0 + 0.6 * global_factor

                        new_rel_x = rel_x * stretch * thickness_scale
                        # 縦方向の変形は compress のみを適用して控えめに保つ
                        new_rel_y = rel_y * compress
                        new_px = cx + new_rel_x
                        new_py = cy + new_rel_y
...
class manage_window:
    ...
    def mathematical(c: tuple, p: tuple, r: float):
        ...
        # X方向へのスケーリングを優先し、Y方向のスケーリングは控えめにする
        # これにより、伝播で過度にY方向へ伸びるのを防止する
        y_scale = 1.0 + 0.25 * (right_p - 1.0)
        right = (right_p * mines[0], y_scale * mines[1])
