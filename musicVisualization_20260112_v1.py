...
print(f"検出された音符数: {len(midi_notes)}")
# --- F0スムージング終了 ---
...
class config:
    ...
    random_color = True
    random_size = False
    fixed_size = 15  # サイズを少し小さくして、より多くの円を描画可能に
    @staticmethod
    def racalc():
        pass
...
def initialize_marbling_with_colors():
    ...
    if bpm > 0 and len(circles) > 0:
        ...
        for x_break in x_positions:
            ...
            sorted_circles = sorted(circles, key=lambda c: c.position[1])
            prev_shift = shift
            ...
            for c in sorted_circles:
                ...
                if abs(center_x - x_break) < break_range:
                    ...
                    c.points = new_points
                    # 位置も更新
                    c.position = (c.position[0] + this_shift, c.position[1])
...
class manage_window:
    ...
    def redraw_circle():
        ...
        # ジャンル名とBPMを描画
        try:
            canvas.create_text(10, 10, text=f"{genre_list[predicted_label]}  BPM:{bpm:.2f}", 
                             fill="black", font=("Arial", 16, "bold"), anchor="nw")
